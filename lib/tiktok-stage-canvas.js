...
// `px-3` (padding kiri/kanan, TETAP relatif ke stage -- gak ikut zoom) +
// `max-w-[272px]` (yang IKUT di-zoom lewat `zoomFactor`, sama seperti CSS
// `transform: scale()` di preview -- lihat TiktokStage), diskalakan dari
...
export function computeCardLayout(stageW, stageH, refW = STAGE_REF_W, zoomFactor = 1) {
  const stageScale = stageW / refW;
  const cardScale = stageScale * zoomFactor;
  const sidePad = CARD_SIDE_PADDING * stageScale;
  const maxCardW = CARD_REF_W * cardScale;
  const cardW = Math.min(stageW - sidePad * 2, maxCardW);
  const cardH = computeCardHeight(cardW, cardScale);
  return {
    cardX: (stageW - cardW) / 2,
    cardY: (stageH - cardH) / 2,
    cardW,
    cardH,
    scaleFromRef: cardScale,
  };
}
...
//
// `state.cardZoom` (opsional, persen -- 100 = normal): posisi maju/mundur
// card, IKUT ke-export sama persis kayak preview (lihat cardZoom di
// TiktokStage) -- makin kecil dari 100 = card mengecil/mundur (background
// makin kelihatan), makin gede = card membesar/maju. HANYA memengaruhi
// ukuran card & metrik internalnya, background TETAP diskalakan dari
// stageScale murni supaya blur-nya gak ikut berubah pas card di-zoom.
export function drawStageFrame(ctx, stageW, stageH, state) {
  const refW = state.refStageWidth || STAGE_REF_W;
  const stageScale = stageW / refW;
  drawStageBackground(ctx, stageW, stageH, state.coverImg, state.bgBlur, stageScale);
  const zoomFactor = (state.cardZoom || 100) / 100;
  const layout = computeCardLayout(stageW, stageH, refW, zoomFactor);
